% Description: Compute optical flow between pairs of images, and compute motion compensated
...
%% Calculate Optical flow velocity vectors and cost function
...
E0 = img2 - img1; % Initial error

I_tilda = lin_sep_interpolation(img1, u, v); % Using linear interpolation to get
% values of image after shifting the image by velocity vectors obtained by optical flow
EN = img2 - I_tilda; % Cost function after optical flow
imshow(EN)
...
figure; imshow(imread('Fall_trees_0.tif'));
figure; imshow(imread('Fall_trees_0.5.tif'));
figure; [t,w] = meshgrid(1:rows, 1:cols);
quiver(w,t,u(:,:,1)', v(:,:,1)');
figure; quiver(w,t,u(:,:,10)', v(:,:,10)');
figure; quiver(w,t,u(:,:,50)', v(:,:,50)');
figure; quiver(w,t,u(:,:,200)', v(:,:,200)');
camroll(180)
